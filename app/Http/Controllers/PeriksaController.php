...
{
    $jadwals = Jadwal::where('tersedia', true)
    ->where('waktu', '>', now())
    ->with('dokter') // pastikan relasi dokter ada
    ->get();


    return view('pasien.periksa.create', compact('jadwals'));
}
...
public function pasienStore(Request $request)
{
    ...
    // Buat periksa berdasarkan jadwal yang dipilih
    Periksa::create([
        'id_pasien' => auth()->id(),
        'id_dokter' => $jadwal->id_dokter,
        'tgl_periksa' => $jadwal->waktu,
        'jadwal_id' => $jadwal->id,
        'catatan' => null,
        'biaya_periksa' => null,
    ]);

    // Tandai jadwal sudah tidak tersedia
